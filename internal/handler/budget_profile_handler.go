...
func (h *BudgetHandler) SetBudgetAutoUpdatePlannedAmount(ctx context.Context, req *connect.Request[v1.SetBudgetAutoUpdatePlannedAmountRequest]) (*connect.Response[v1.SetBudgetAutoUpdatePlannedAmountResponse], error) {
	userID, err := h.currentUserID(ctx)
	if err != nil {
		return nil, err
	}
	...
	profile, svcErr := h.profiles.SetAutoUpdatePlannedAmount(ctx, id, userID, req.Msg.Enabled)
	if svcErr != nil {
		return nil, toConnectError(svcErr)
	}
	return connect.NewResponse(&v1.SetBudgetAutoUpdatePlannedAmountResponse{Profile: toProtoBudgetProfile(profile)}), nil
}
...
func toProtoBudgetProfile(p db.BudgetProfile) *v1.BudgetProfile {
	return &v1.BudgetProfile{
		...
		AutoUpdatePlannedAmount: p.AutoUpdatePlannedAmount,
	}
}
